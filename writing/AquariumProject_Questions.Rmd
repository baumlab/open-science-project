...
JR: I think what we are trying to do here is to make predictions about aquarium trade based on 1) exporting country information, such as governance factors (CITES, development), and 2) species information, such as trophic group, range size. Trade volume is a big problem just now, because it is very skewed towards a couple of major exporters (PNG and Philippines). There may also be discrepepancies in volume estimates due to incorrect labelling of fish + exported boxes (Rhyne et al. 2012), but this seems less of an issue if we just analyse which species each country exports.

So, I suggest that we put volumes to one side and simplify this question to focus on species incidences - our response variable is the list of species exported per country.

We probably need to control for biogeography here as well. Caribbean species can be exported by X countries, whereas Indo-Pacific species can be exported by Y countries - we want to measure effect of predictors *given* that there is a geographical constraint on how many countries can export each species. Kulbicki et al. (2013, PLoS ONE) has a dataset of biogeographical zones we could use.
...
**Hypotheses**: Countries with poor governance will have higher trade export diversity (standardised to reef area/coastline/regional richness). 
...
**Hypotheses**: The planktivores are primarily targeted because they are wanted by the aquarium market due to brilliant colours and 

planktivores are primarily targeted. Are there 1) countries that export more herbs/piscs than expected? 2) Which non-planktivore species are exported in high volumes? 
...
**Hypotheses**: There will be a positive linear relationship of export occurences (i.e. number of countries exporting that species) and range size, as species with a larger range size will be exported by more countries. However if any species fall of this line they are of interest: 1) smaller range size for volume = high risk; 2) larger range size than volume = low risk?

**Data**: OBIS range sizes (from this calculated EOO: extent of occurence), Luis et al. 2013 range size data. The figures below are for EOO

JR: This would change to export diversity (= number of countries exporting that species) ~ range size.
...
```
glmer(volume ~ range size + (1 + range size | year / country), data=trade, family = Gamma(link=log))  
```
* random slope for country OR biogeographic region, random intercept for year  
* Gamma distribution= volume is positive non-integer, maybe compare with log(volume) and normal distribution  
* if we look at export diversity, use poisson
* or could try gls() variance structures? (varfixed, varpower) instead of transforming  

```
glmer(volume ~ range size + (1| year / country), data=trade, family = Gamma(link=log))  
``` 
...
Volume models:
```
glmer(volume ~ range size + CITES + gov.indicator + (1 + range size | year / region), data=trade, family = Gamma(link=log))  
```

Incidence models:
```
glmer(export_diversity ~ range size + CITES + gov.indicator + (1 + range size | year / region), data=trade, family = Gamma(link=log))  
```
