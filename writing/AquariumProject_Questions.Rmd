...
Response variable: If a species was exported or not (0,1) (binomial).
...
**Hypotheses**: The planktivores are primarily targeted because?

**Data**: CREP trophic groups (lots of data missing for our species), FishBase (75 of our 100 species matched), JPWR looking for another dataset  
...
Volume models:
```
glmer(ocurrence ~ range size + CITES + gov.indicator + (1 + range size | year / region), data=trade, family = Gamma(link=log))  
```

Incidence models:
```
glmer(occurence ~ range size + CITES + gov.indicator + (1 + range size | year / region), data=trade, family = Gamma(link=log))  
```
