...
table(clams$lat,clams$lon)

all_locations=expand.grid(clams$lat,clams$lon)
all_locations_unique =unique(all_locations)

clams$site_code=NULL
for (q in 1:nrow(clams)){
  clams$site_code[q] = which(all_locations_unique$Var1==clams$lat[q] & all_locations_unique$Var2==clams$lon[q])
}

library(MASS)
library(glmmADMB)

clams$site_code=as.factor(clams$site_code)
model = glmmadmb(formula=sum~pdo+year+spill+(1|site_code),data=clams,family="poisson",extra.args="-ndi 30000",zeroInflation=FALSE)
